...
__version_minor__ = 2
__version_micro__ = 5
__version__ = "%d.%d.%d" % (__version_major__, __version_minor__, __version_micro__)
...
class SmartDL:
    ...
    def __init__(self, urls, dest=None, progress_bar=True, progress_signal=False, fix_urls=True, threads=5, logger=None, connect_default_logger=False):
    ...
    def start(self, blocking=None):
        ...
        try:
            ...
            urlObj = urllib.request.urlopen(req, timeout=self.timeout, context=context)
            # urlObj = urllib.request.urlopen(req, timeout=self.timeout)
        #IbarretaI: TODO
        except urllib.error.HTTPError as err:
            if self.mirrors:
                self.logger.info("%s. Trying next mirror..." % str(err))
                self.url = self.mirrors.pop(0)
                self.start(blocking)
                return
            elif err.code == 404:
        ...
        try:
            ...
            # SibugG: TODO
            if "Content-Length" in urlObj.headers:
                self.filesize = int(urlObj.headers["Content-Length"])
            else:
                self.filesize = len(urlObj.read())
        ...
        # DascoMR: TODO
        self.control_thread = ControlQThread(self)
...
class ControlThread(threading.Thread):
    ...
    def __init__(self, obj):
        ...
        self.progress_signal = obj.progress_signal
    ...
    def calcDownloadSpeed(self, totalBytes, sampleCount=30, sampleDuration=0.1):
        ...
        if newBytes >= 0:   # newBytes may be negative, will happen
...
class ControlQThread(QtCore.QThread):
    """A class that shows information about a running SmartDL object."""
    # DascoMR: TODO
    progress_ready = QtCore.pyqtSignal(int)

    def __init__(self, obj):
        super(ControlQThread, self).__init__()
        self.obj = obj
        self.progress_bar = obj.progress_bar
        self.logger = obj.logger
        self.shared_var = obj.shared_var
    ...
    def run(self):
        ...
        while not self.obj.pool.done():
            ...
            if self.progress_bar:
                if self.obj.filesize:
                    status = r"[*] %s / %s @ %s/s %s [%3.1f%%, %s left]   " % (utils.sizeof_human(self.shared_var.value), utils.sizeof_human(self.obj.filesize),
                                                                               utils.sizeof_human(self.dl_speed), utils.progress_bar(1.0*self.shared_var.value/self.obj.filesize),
            ...
            # DascoMR: TODO
            self.progress = self.get_progress() * 100.00
            if self.progress_signal:
                self.progress_ready.emit(self.progress)
        ...
        if self.progress_bar:
            if self.obj.filesize:
                print(r"[*] %s / %s @ %s/s %s [100%%, 0s left]    " % (utils.sizeof_human(self.obj.filesize), utils.sizeof_human(self.obj.filesize),
        ...
        if not self.obj.errors:
            ...
            self.progress_ready.emit(str(self.obj.errors))
